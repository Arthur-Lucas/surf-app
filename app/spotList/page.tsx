...
const numericSortFields: (keyof Spot['stats'])[] = ['swellHeight', 'wind', 'tide', 'rating'];
...
export default function SpotsPage() {
    ...
    const sortSpots = (criteria: keyof Spot['stats']) => {
        const sorted = [...spots].sort((a, b) => {
            ...
            const aVal = typeof aRaw === 'string' ? parseFloat(aRaw.replace(/[^\d.]/g, '')) : aRaw;
            const bVal = typeof bRaw === 'string' ? parseFloat(bRaw.replace(/[^\d.]/g, '')) : bRaw;
            return bVal - aVal;
        });
        setSpots(sorted);
        setSortBy(criteria);
    ...
    const filteredSpots = onlyFavorites ? spots.filter(spot => favorites.includes(spot.id)) : spots;

    return (
        <main className="bg-white text-black p-6 min-h-screen">
            <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                <div className="flex gap-2 flex-wrap">
                    <Button variant={sortBy === 'swellHeight' ? 'default' : 'outline'} onClick={() => sortSpots('swellHeight')}>
                        🌊 Houle
                    </Button>
                    <Button variant={sortBy === 'wind' ? 'default' : 'outline'} onClick={() => sortSpots('wind')}>
                        💨 Vent
                    </Button>
                    <Button variant={sortBy === 'tide' ? 'default' : 'outline'} onClick={() => sortSpots('tide')}>
                        🌙 Marée
                    </Button>
                    <Button variant={sortBy === 'rating' ? 'default' : 'outline'} onClick={() => sortSpots('rating')}>
                        ⭐ Accessibilité
                    </Button>
                    ...
                    >
                        ...
                        <CardContent className="relative p-5 space-y-3 z-10">
                            <div className="flex justify-between items-start">
                                ...
                                >
                                    <Heart fill={favorites.includes(spot.id) ? 'black' : 'white'} stroke="black" />
                                </button>
                            </div>
